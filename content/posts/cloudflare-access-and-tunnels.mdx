...
> Note that `config.yml` placed in the `cloudflared` folder is the default, you can name it something else or have several config files by passing the --config flad followed by the file path for the config file. Ex; `cloudflared tunnel --config ./jarvis-tunnel.yaml run`
...
Once signed up, you'll want to configure an Application under the Access menu for each host we setup earlier in our config file. The process is pretty straightforward and you'll want to setup an access rule to suit your needs. I'd suggest starting off with an email rule and just listing the allowed emails for each app. You can also connect more auth services, but One-time Pin is enough to get us started.
...
Now that we've got everythign working, we'll move the tunnels over to the docker host so that it can run 24/7.

First, we'll need to move the tunnel config file we wrote over to the docker host in an app data folder. I put these at `/home/appdata/cloudflared`. There is a _slight_ adjustment that needs to be made.
...
Here's the docker-compose file I'm using, take note of the version of the cloudflared container (there is no "latest" tag for this container).
